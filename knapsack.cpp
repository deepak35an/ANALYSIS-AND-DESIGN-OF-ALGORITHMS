...
int partitionPW(vector<pair<float,pair<float,float>>>&arr , int low , int high)
{
    int i = low;
    int j = high;
    float pivot = arr[low].first; // Corrected pivot type to float
    while(i < j)
    {
        while(arr[i].first >= pivot && i <= high-1) // Corrected the comparison
            i++;
        while(arr[j].first < pivot && j >= low+1) // Corrected the comparison
            j--;
        if(i < j) 
            swap(arr[i], arr[j]); 
    }
